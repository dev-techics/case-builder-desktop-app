...
const PricingCard = ({
  ...
  return (
    <motion.div
      className={`relative bg-white flex flex-col rounded-2xl p-6 md:p-8 transition-all duration-300 hover:-translate-y-1 ${
        plan.highlighted
    ...
    >
      {/* Badge */}
      {plan.badge && (
        <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-primary text-white px-5 py-1 rounded-full text-xs font-black uppercase tracking-widest shadow-md flex items-center gap-1.5">
            ...
            <div className="relative flex items-center gap-1.5">
              <span
                className={`text-sm text-on-surface ${
                  plan.highlighted ? 'font-semibold' : 'font-medium'
                }`}
              >
                {feature.label}
              </span>
      ...
      {plan.highlighted ? (
        <button
          ...
          onClick={handleCta}
          disabled={isDisabled}
          className="w-full py-3.5 px-4 bg-primary text-white font-bold text-sm tracking-wide rounded-xl hover:bg-primary-container transition-all shadow-md cursor-pointer hover:shadow-primary/20 hover:scale-[1.01] active:scale-[0.98] focus:outline-none focus:ring-4 focus:ring-primary/20 whitespace-nowrap duration-150"
        >
